...
class Rent_non_real_estate extends CI_Controller
{
    ...
    public function add() {
        ...
        if(count($result)>0) {
            ...
            $result=$query->result();
            $data['tax']=$result;
            //$data['property']= $this->rent_model->getPropertyDetails('',2);
            $result = $this->db->query("call sp_getpropertynorent(2,'')")->result();
    ...
    {
        $this->get_record($rent_id, 'Rent_non_real_estate/rent_non_real_estate_view');
    }

    public function edit($rid){
        $this->get_record($rid, 'Rent_non_real_estate/rent_non_real_estate_details');
    }
    ...
    public function get_record($rid, $view){
        ...
        if(count($result)>0) {
            if($result[0]->r_edit==1 or $result[0]->r_approvals==1) {
               ...
               /* if($txn_fkid!=''){
                    $data['property']=$this->purchase_model->purchaseData('All',$txn_fkid,'2');
                } else {
                    $data['property'] = $this->purchase_model->purchaseData('All',$rid,'2');
                }*/
                ...
                if(count($result)>0) {
                    ...
                    $result = $this->db->query("call sp_getpropertynorent(2,$property_id)")->result();
                    mysqli_next_result( $this->db->conn_id );
                    $data['property']=$result; 
